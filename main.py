...
class Calculater(QMainWindow):
    ...
    def get_entry_text_wigth(self) -> int:
        return self.entry.fontMetrics().boundingRect(self.entry.text()).width()

    def get_temp_text_wigth(self) -> int:
    ...
    def disable_buttons(self, disable: bool) -> None:
        ...
        color = 'color: #888;' if disable else 'color_white;'
    ...
    def adjust_entry_font_size(self) -> None:
        ...
        while self.get_entry_text_wigth() > self.entry.width() - 15:
            font_size -= 1
            self.entry.setStyleSheet('font-size: ' + str(font_size) + 'pt; border: none;')

        font_size = 1
        while self.get_entry_text_wigth() < self.entry.width() - 60:
            font_size += 1
    ...
    def adjust_temp_font_size(self) -> None:
        ...
        while self.get_temp_text_wigth() > self.temp.width() - 10:
            font_size -= 1
            self.temp.setStyleSheet('font-size: ' + str(font_size) + 'pt; color: #888;')

        font_size = 1
        while self.get_temp_text_wigth() < self.temp.width() - 60:
            font_size += 1
